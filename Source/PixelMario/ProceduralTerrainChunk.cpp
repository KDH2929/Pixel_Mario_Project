...
void AProceduralTerrainChunk::SetPoilageActor(TSubclassOf<AActor> InActor)
{
	PoilageActor = InActor;
}
...
void AProceduralTerrainChunk::SpawnPoilages()
{
	if (PoilageActor)
	{
		for (const FVector& Vertex : Vertices)
		{
			...
			AActor* SpawnedActor = GetWorld()->SpawnActor<AActor>(PoilageActor, ActorLocation, FRotator::ZeroRotator, SpawnParams);
...
/*   Runtime Poilage 생성   (컴퓨터 사양 이슈로 포기)

if (SpawnedActor)
			{
				AProceduralPoilage* Poilage = Cast<AProceduralPoilage>(SpawnedActor);

				if (Poilage)
				{
					if (Poilage->GetStaticMesh())
					{
						FVector StaticMeshBoxExtent = Poilage->GetStaticMesh()->GetBoundingBox().GetExtent();
						...
						// Poilage->SetRowColumn(NumRow, NumColumn);
						Poilage->SpawnPoilage();
